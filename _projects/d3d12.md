...
This was a self-directed project spread across two blocks, built entirely outside of regular project hours. The goal was to get hands-on experience with a modern low-level graphics API, starting from nothing and building up to a renderer capable of loading models with textures and lighting.

## Block B – Building the Framework

The first block was spent purely on getting a working foundation in place. Coming from OpenGL in Year 1 and compute shaders in Godot, DirectX 12's explicit architecture was a significant jump. I followed Jeremiah's 3dgep tutorials as the primary resource, supplemented by Microsoft's DX12 samples for filling in gaps the tutorials skipped.
...
Blinn-Phong lighting with point and directional lights was implemented next, referencing LearnOpenGL's lighting chapters and adapting the equations for HLSL. This was the first time the lighting math properly clicked for me — understanding why Blinn-Phong is an approximation of PBR specular and why the Phong model itself is largely a hack. Normal mapping followed, using the TBN matrix with pre-calculated tangents and bitangents from Assimp. Emissive mapping was straightforward on top of that.
